    ...
    def __call__(self, cur_image, prev_image):
        ...
        for i, theta in enumerate(self.thetas):
            if i<4:
                ...
                Energy_SUMs.append(0.5*Energy_ON + Energy_OFF)
        ...
        # PV5 = 1.0 * PV5_OFF_0 + 0.5*PV5_ON_1
        # PV5 = (0.5*PV5_OFF_0 + 0.0*PV5_ON_1) + (1.0*PV5_ON_0 + 0.0*PV5_OFF_1)
        PV5 = (1.0 * PV5_OFF_0 + 0.2 * PV5_ON_1) + (0.2 * PV5_ON_0 + 0.1 * PV5_OFF_1)
        ...
        Saliency_Mask1 = (Saliency_map1 > 0.005) * 1.
        ...
        Saliency_Mask2 = (Saliency_map2 > 0.006) * 1.
...
if __name__ == '__main__':
    ...
    args.log_dir = args.log_dir + '_ASNN_clbrobot'

    # # Create log path
    #if not os.path.isdir(args.log_dir):
    #    os.makedirs(args.log_dir)

    videoCapture = cv2.VideoCapture('data/camera_image_raw.mp4')

    width = 120
    height = 90
    
    retina = Retina(width, height)
    print(width, height)
    Sfs = []

    success, frame = videoCapture.read()
    prev_frame = cv2.resize(frame, (width, height), interpolation=cv2.INTER_CUBIC)
    ...
    while success:
        ...
        if success and i%2==0:
            ...
            pv5, pv5_new, motionenergy, mask1, mask2, energy_looming, Dx, Dy, estimated_pos, estimated_direction, thetas, motionenergy_sum = retina(
                frame, prev_frame)
            prev_frame = frame.copy()
            Sfs.append(energy_looming)
            print(i)
            # Plotting
            X = np.arange(0, width)
            Y = np.arange(0, height)
            sample = 2
            IndexX = np.arange(0, width, sample)
            IndexY = np.arange(0, height, sample)
            l = sample / 2.
            ...
            ax1.imshow(mask2, cmap = cm.Reds, alpha=0.4, interpolation = "nearest")
            ax1.quiver(X[::sample], Y[::sample], -Dx[::sample, ::sample], Dy[::sample, ::sample], scale=5.0,
                        color='r', angles='xy', pivot='middle', headwidth=3, headlength=3)
            ...
            if estimated_pos != []:
                ...
                ax1.quiver(estimated_pos[:,0], estimated_pos[:,1], -estimated_direction[:,0], estimated_direction[:,1], scale=0.3, color='b', angles='xy', headwidth=3,
                            headlength=3)

                mean_estimated_pos = estimated_pos.mean(0)
                
                if energy_looming>78:
                    scale = energy_looming/3
                    if mean_estimated_pos[0]<=(width/3.):
                        print('go back and turn right: %f'%scale)
                    elif mean_estimated_pos[0]>=(4*width/3.):
                        print('go back and turn left: %f'%scale)
                    else:
                        print('go back and stop %f'%scale)
            ...
            ax2.plot(Sfs)
            ax2.set_ylim([-1, 1000])
            # plt.savefig(args.log_dir +'/asnn_clbrobot_%d.png' % i, bbox_inches='tight', dpi=400)
            plt.draw()
            plt.show()
            plt.pause(0.01)
        if not success:
    ...
    #LoomingEnergys = {"LoomingEnergy": Sfs}
    ...
    # plt.plot(Sfs)
    # plt.ylim([-1, 800])
    # plt.savefig(args.log_dir+'/asnn_mobile_robot.pdf', bbox_inches='tight', dpi=400)
    plt.show()